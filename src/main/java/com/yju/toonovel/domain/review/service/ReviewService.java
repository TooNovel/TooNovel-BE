...
import com.yju.toonovel.domain.novel.exception.GenreNotFoundException;
...
public class ReviewService {
	...
	//리뷰 등록
	public void reviewRegister(ReviewRegisterRequestDto dto) {
		User user = userRepository.findByUserId(dto.getWriterId())
	...
	public void reviewDelete(Long reviewId) {
		reviewRepository.findById(reviewId)
			.ifPresentOrElse(
				review -> reviewRepository.deleteById(reviewId),
				() -> {
	...
	public Page<AllReviewInWorkResponseDto> getAllReviewWithLike(Long nid, Long uid) {

		Pageable pageable = PageRequest.of(0, 10);
		Page<AllReviewInWorkResponseDto> reviewList = reviewRepository
			.getReviewAndReviewLikeByNovelOrderByReviewIdPaging(nid, uid, pageable);
